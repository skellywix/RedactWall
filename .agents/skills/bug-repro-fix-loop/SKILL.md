...
3. **Isolate** the cause in the module (`detection-engine/detect.js`, `server/policy.js`, …). Form a one-line hypothesis before editing.
4. **Fix** minimally. If in `detection-engine/`, run `npm run sync-engine` to keep `sensors/browser-extension/lib/` identical.
