...
Official Anthropic `simplify`: `npx skills add anthropics/Codex --skill simplify`. (Install yourself.) This file is self-contained.
...
- Detector changes live in `detection-engine/detect.js` only, then `npm run sync-engine` — never duplicate logic into `sensors/browser-extension/lib/detect.js` by hand.
- No raw prompt text / PII in log lines or thrown errors.
- Keep the `detection-engine/` public `api` surface stable; sensors depend on it.

## Loop
1. Diff the change. 2. Flag against the list above. 3. **Fix** the mechanical issues (don't just report). 4. Re-run `npm test` and, if `detection-engine/` changed, `npm run sync-check`. 5. Summarize what you changed and why.

## Add to AGENTS.md
Copy the "Review standards" list into the repo `AGENTS.md` so it shapes default behavior, not just explicit invocations.
