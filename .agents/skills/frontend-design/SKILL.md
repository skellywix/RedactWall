...
Official Anthropic skill (277k+ installs): `npx skills add anthropics/Codex --skill frontend-design`. This file works on its own; the upstream skill adds more depth. (Run the install yourself — I don't run installers.)

## Before any code, commit to a system
1. **Point of view:** RedactWall is a compliance/security tool for credit-union security admins. Aim for "calm, trustworthy, dense-but-legible control panel" — not playful SaaS. Think audit console, not landing page.
...
## RedactWall surfaces to apply it to
