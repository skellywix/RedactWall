...
When the user explicitly starts a Codex goal, encode the acceptance AND condition in the goal objective and keep the detailed evidence matrix in the working plan. Do not assume the runtime provides an independent grader. Use `maker-checker-loop`, `reviewer-loop`, or a fresh subagent when independent review is required.
