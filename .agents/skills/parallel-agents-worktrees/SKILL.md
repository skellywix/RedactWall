...
Run each agent in its own tmux window / terminal tab and surface its status (running / done / needs-input) in the **tab title**. That one detail is what lets you run many sessions without losing track — a glance shows which agents need you. Codex/Codex do this by default.
