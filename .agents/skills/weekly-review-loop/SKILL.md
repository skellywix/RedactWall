...
2. **Read what you didn't write.** For each agent-authored merge touching `detection-engine/detect.js`, `server/policy.js`, `server/db.js`, or `server/crypto.js`, summarize WHAT changed and WHY in one line. Anything you can't explain → add to `STATUS.md` as `comprehension-debt` to review with a human.
3. **Full verification sweep:**
   ...
   - `node -e "require('./server/db').verifyAuditChain()"` (tamper-evident log intact)
