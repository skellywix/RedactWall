...
| `pr-review-loop` | Adversarial pre-human review: quality + PromptSentinel invariants (no PII in logs, engine parity, `alwaysBlock` intact). |
...
Installers were intentionally NOT run. The wrapper SKILL.md files document *when to use* each tool and a PromptSentinel-tuned playbook; run the install command yourself, and provide any API keys / OAuth consent directly (never share secrets in chat).
...
Companion file: **`AGENTS.md`** (repo root) — the agent operating contract: commands, hard invariants, how to exercise PromptSentinel E2E, review standards, and a mistake log. `agent-self-validation` and `delegate-like-a-manager` depend on it.
