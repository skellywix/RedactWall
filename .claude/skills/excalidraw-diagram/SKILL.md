...
description: Generate production-quality Excalidraw architecture/sequence diagrams from natural language, with a render-and-self-check loop so the output isn't broken. For PromptSentinel, use it to draw three-sensors-one-brain, the block→approve flow, and the audit hash-chain. Wraps the external excalidraw-diagram skill.
---
