...
| Field | Weak | Verifiable (PromptSentinel example) |
|-------|------|-------------------------------------|
...
| Constraints | (unstated) | "Do not change `shared/`'s public `api`; do not weaken any `alwaysBlock` type; keep `npm run sync-check` green" |
...
4. Re-check **every constraint** (here: `npm run sync-check` for engine parity; `node -e "require('./src/db').verifyAuditChain()"` if audit code changed; `npm run train-semantic && npm run sync-check` if you touched the semantic model — CI fails on drift).
