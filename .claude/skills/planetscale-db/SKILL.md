---
name: planetscale-db
description: Index-aware schema design and git-style DB branching for serverless MySQL/Postgres via the pscale CLI. Directly relevant to RedactWall's roadmap item of moving off single-node SQLite to a "real database". Use when designing the production data layer for audit, queries, and approvals. Wraps the external PlanetScale skill.
---

# PlanetScale Database Skills

Teaches the agent to design schemas that scale and to treat schema changes as reviewable, branchable, reversible code — the discipline RedactWall needs as it graduates from `better-sqlite3`.
...
## RedactWall context (the roadmap "real database")
Today state lives in `data/redactwall.db` (SQLite via `better-sqlite3`), with the tamper-evident, hash-chained audit table central to the compliance story. When migrating:
...
- **Branch per change:** `pscale branch create redactwall <feature>`, design, `pscale deploy-request create` — every schema change is reviewed like a PR.
