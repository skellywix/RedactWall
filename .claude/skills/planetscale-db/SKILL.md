---
name: planetscale-db
description: Index-aware schema design and git-style DB branching for serverless MySQL/Postgres via the pscale CLI. Directly relevant to PromptSentinel's roadmap item of moving off single-node SQLite to a "real database". Use when designing the production data layer for audit, queries, and approvals. Wraps the external PlanetScale skill.
---

# PlanetScale Database Skills

Teaches the agent to design schemas that scale and to treat schema changes as reviewable, branchable, reversible code — the discipline PromptSentinel needs as it graduates from `better-sqlite3`.
...
## PromptSentinel context (the roadmap "real database")
...
- **Branch per change:** `pscale branch create promptsentinel <feature>`, design, `pscale deploy-request create` — every schema change is reviewed like a PR.
