...
1. `shared/detect.js` — every detector + the semantic categories (`SOURCE_CODE`, `CONFIDENTIAL_BUSINESS`, `CREDENTIALS`, `LEGAL_CONTRACT`). Cover both true positives AND the false-positive guards (e.g. bare 9-digit number is NOT a routing number without context).
2. `src/policy.js` — `evaluate()` decisions across all modes (warn / justify / redact / block) and the `alwaysBlock` hard stops.
3. `src/crypto.js` — `seal`/`open` round-trip and the disabled-key path.
4. `src/db.js` — `appendAudit` + `verifyAuditChain` detects tampering (mutate an entry, expect `ok:false`).
...
5. `npm run sync-check` if you touched anything under `shared/`. Commit. Update `STATUS.md`.
