...
- Follow with higher-friction collectors only after the MVP proves value:
  native messaging from browser upload surfaces or app-specific collectors.
...
- Manual Windows smoke with synthetic clipboard text: clipboard guard reports
  masked evidence only, `--clear-on-block` clears locally, and
  `verifyAuditChain()` returns `ok:true`.
...
file-upload blocks, destination-scoped browser paste, file-drop, and
response-copy blocks, detector ignores, scanner controls, server-side
...
- Clipboard guard records only masked findings and clears locally when
  `--clear-on-block` is enabled.
