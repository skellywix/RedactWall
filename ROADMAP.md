...
| N2 | ✅ **Examiner report pack — SHIPPED (core slices).** The schemaVersion-3 examiner-profile evidence pack maps AI usage, enforcement outcomes, and evidence artifacts to NCUA Part 748 Appendix A, GLBA Safeguards Rule 501(b), and NIST AI RMF (`server/evidence.js`, `server/control-map.js`, `scripts/export-evidence-pack.js --examiner-profile federal_credit_union`). **Open tails:** FFIEC handbook control-family labels (not yet coded) and a rendered human-readable report (JSON today). | None — this is the wedge | NCUA examiners probe AI inventories and policy-to-evidence traceability in 2026 exams. | S (tails only) |
