...
function summaryText(map: LeakMapReport | null): string {
  ...
  const s = map.summary;
  if (!num(s.events)) return `No sanitized data activity yet - the map draws as sensors report flows / ${s.privacy || 'prompt bodies excluded'}`;
  ...
  return (
    <section className="leak-map-section" aria-label="AI data leak exposure map">
      ...
      <div className={`leak-map-stage${still ? ' is-static' : ''}`} id="leakMapStage">
        {model && map ? (
          <svg viewBox={`0 0 ${W} ${model.height}`} role="img" aria-label="Map of sensitive-data paths from institution teams through RedactWall to AI destinations">
            <text className="leak-col-label" x={10} y={24}>INSTITUTION TEAMS</text>
          ...
          <div className="leak-map-empty">
            ...
            <p>Connect sensors and the exposure map draws every institution team-to-AI flow from sanitized events.</p>
          </div>
        )}
      </div>
      {inspector}
