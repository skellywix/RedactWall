...
PromptWall cannot inspect the prompt or response.
...
- Redact-mode tokenized prompt forwarding when the control plane returns a
  `tokenizedPrompt`.
- Approval hold polling when `PROMPTWALL_GATEWAY_APPROVAL_WAIT_MS` is set.
...
- Request correlation through `X-PromptWall-Request-Id`, rate-limit headers,
  `/healthz`, and `/readyz`.

## Run Locally

Start PromptWall first:
...
```powershell
$env:PROMPTWALL_GATEWAY_TOKEN = "replace-with-client-token"
$env:PROMPTWALL_GATEWAY_UPSTREAM_API_KEY = "sk-or-provider-key"
...
```powershell
$env:PROMPTWALL_GATEWAY_RATE_LIMIT_STORE = "sqlite"
$env:PROMPTWALL_GATEWAY_RATE_LIMIT_DB = "C:\PromptWall\data\gateway-rate-limits.db"
node scripts/ai-llm-gateway.js --sentinel http://127.0.0.1:4000 --upstream https://api.openai.com --port 4182
```
...
The load balancer publishes only `PROMPTWALL_GATEWAY_PUBLIC_PORT` (default
...
per hashed key, and stores only `PROMPTWALL_RATE_LIMITER_REDIS_PREFIX` plus the
...
before calling the PromptWall control plane or the upstream model provider.
...
```http
Authorization: Bearer replace-with-client-token
X-PromptWall-User: analyst@example.test
Content-Type: application/json
```

The gateway strips caller auth before the upstream request and injects
`PROMPTWALL_GATEWAY_UPSTREAM_API_KEY` as the provider credential.
...
| --- | --- | --- | --- |
| `--token` | `PROMPTWALL_GATEWAY_TOKEN` | none | Required client token. Comma-separated env values are allowed. |
| `--host` | `PROMPTWALL_GATEWAY_HOST` | `127.0.0.1` | Bind address. Use `0.0.0.0` only inside a private container or behind a trusted load balancer. |
| `--upstream` | `PROMPTWALL_GATEWAY_UPSTREAM` | `https://api.openai.com` | Provider base URL. |
| `--upstream-key` | `PROMPTWALL_GATEWAY_UPSTREAM_API_KEY` | none | Provider API key the gateway uses upstream. |
| `--upstream-auth-header` | `PROMPTWALL_GATEWAY_UPSTREAM_AUTH_HEADER` | `authorization` | Header that carries the upstream provider key, for example `authorization`, `x-api-key`, or `x-goog-api-key`. |
| `--upstream-auth-scheme` | `PROMPTWALL_GATEWAY_UPSTREAM_AUTH_SCHEME` | `Bearer` | Prefix before the upstream key. Use `none` for raw key headers such as `x-goog-api-key`, or `aws-sigv4` for direct Bedrock Runtime signing. |
| `--aws-region` | `PROMPTWALL_GATEWAY_AWS_REGION`, `AWS_REGION`, or `AWS_DEFAULT_REGION` | none | AWS region for `aws-sigv4` upstream signing. |
| `--aws-service` | `PROMPTWALL_GATEWAY_AWS_SERVICE` | `bedrock` | AWS service name for SigV4 signing. |
| `--upstream-header` | `PROMPTWALL_GATEWAY_UPSTREAM_HEADERS` | none | Bounded static upstream header in `Name=Value` form. Repeat the flag or comma-separate env values for provider version headers. |
| `--approval-wait-ms` | `PROMPTWALL_GATEWAY_APPROVAL_WAIT_MS` | `0` | How long to poll held prompts before returning 403. |
| `--rate-limit` | `PROMPTWALL_GATEWAY_RATE_LIMIT` | `60` | Requests per client token per window. |
| `--rate-window-ms` | `PROMPTWALL_GATEWAY_RATE_WINDOW_MS` | `60000` | Rate limit window. |
| `--rate-store` | `PROMPTWALL_GATEWAY_RATE_LIMIT_STORE` | `memory` | Rate limiter store. Use `sqlite` to share counters across local gateway workers, or `http` to delegate to a shared limiter service. |
| `--rate-db-path` | `PROMPTWALL_GATEWAY_RATE_LIMIT_DB` | `./data/gateway-rate-limits.db` | SQLite rate-limit database path when `--rate-store sqlite` is used. |
| `--rate-url` | `PROMPTWALL_GATEWAY_RATE_LIMIT_URL` | none | HTTPS endpoint for the shared limiter service when `--rate-store http` is used. |
| `--rate-token` | `PROMPTWALL_GATEWAY_RATE_LIMIT_TOKEN` | none | Bearer token sent to the shared limiter service. |
| `--rate-timeout-ms` | `PROMPTWALL_GATEWAY_RATE_LIMIT_TIMEOUT_MS` | `2000` | Timeout for shared limiter checks. |
| `--allowed-models` | `PROMPTWALL_GATEWAY_ALLOWED_MODELS` | all models | Comma-separated model names or wildcard patterns, such as `gpt-4o-mini,company-*`. |
...
| Option | Env | Default | Purpose |
| --- | --- | --- | --- |
| `--token` | `PROMPTWALL_RATE_LIMITER_TOKEN` or `PROMPTWALL_GATEWAY_RATE_LIMIT_TOKEN` | none | Bearer token required from gateway replicas. |
| `--store` | `PROMPTWALL_RATE_LIMITER_STORE` | `sqlite` | Shared limiter backend. Use `sqlite` for one limiter service or `redis`/`valkey` for active-active limiter replicas. |
| `--db` | `PROMPTWALL_RATE_LIMITER_DB` | `./data/gateway-shared-rate-limiter.db` | SQLite database for hashed limiter counters. |
| `--redis-url` | `PROMPTWALL_RATE_LIMITER_REDIS_URL` | none | Redis or Valkey URL for active-active limiter replicas, such as `rediss://:<password>@redis.internal:6380/0`. |
| `--redis-prefix` | `PROMPTWALL_RATE_LIMITER_REDIS_PREFIX` | `promptwall:gateway:rl:` | Prefix prepended to hashed limiter keys. Unsafe characters are stripped. |
| `--redis-timeout-ms` | `PROMPTWALL_RATE_LIMITER_REDIS_TIMEOUT_MS` | `2000` | Timeout for Redis limiter commands. |
...
fail-closed PromptWall outages, prompt redaction before upstream, response
