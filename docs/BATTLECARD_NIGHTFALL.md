...
invent performance numbers — cite `docs/DETECTION_BENCHMARKS.md`.
...
| Claim | Proof |
| --- | --- |
| **Zero egress.** Prompts are classified on-device; nothing leaves the institution to be scanned. Nightfall routes content through their US-only AWS and retains uploaded dev-platform files encrypted for 24h. | `detection-engine/detect.js` runs in the browser/endpoint/gateway; `docs/CUSTOMER_LICENSING.md` "fully functional with zero egress" |
...
| **No per-scan rate limit.** Detection is an in-process function call, not a metered API. Nightfall rate-limits customers to 5–10 rps. | `docs/DETECTION_BENCHMARKS.md` (in-process p95; state the apples-to-oranges caveat vs their cloud figure) |
| **Transparent, per-seat annual pricing.** No usage metering. Nightfall meters on users/apps/TB-scanned/API-calls with masked list prices. | `docs/CUSTOMER_LICENSING.md`; offline Ed25519 license (`server/license.js`) |
| **Light rollout.** Browser force-install policy; no MDM Full-Disk-Access mobileconfig required to start. | `docs/MANAGED_EXTENSION_DEPLOYMENT.md`, `npm run release:extension:check` |
...
| They say | We answer |
| --- | --- |
| "100+ detectors vs your ~44." | Ours are checksum- and context-validated with **published, CI-enforced floors** (`npm run eval`, `docs/DETECTION_BENCHMARKS.md`) and EDM + custom detector packs. Count is not precision; a DLP that cries wolf gets turned off. |
