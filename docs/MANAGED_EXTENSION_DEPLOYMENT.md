...
1. Package and publish the extension through a controlled channel.
...
2. Force-install the extension with Chrome Enterprise policy.
3. Set managed storage values for:
   - `serverUrl`
   - `ingestKey`
   - `orgId`
   - `email` or `user`
4. Confirm the dashboard receives attributed events.
5. Confirm the extension is active on governed AI destinations.
