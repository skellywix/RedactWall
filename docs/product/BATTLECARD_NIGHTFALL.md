...
| Claim | Proof |
| --- | --- |
| **Zero egress (air-gapped SKU).** In the default air-gapped deployment, prompts are classified on-device; nothing leaves the institution to be scanned. Nightfall routes content through their US-only AWS and retains uploaded dev-platform files encrypted for 24h. (RedactWall also offers an opt-in connected SKU with a vendor-side second layer for shops that want it — the difference is it's *your* choice, not the only option.) | `detection-engine/detect.js` runs in the browser/endpoint/gateway; `docs/process/CUSTOMER_LICENSING.md` "fully functional with zero egress"; connected mode: `docs/process/CONNECTED_DEPLOYMENT.md` |
