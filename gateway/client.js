...
function makeClient({ controlPlaneUrl, ingestKey, requestTimeoutMs }) {
  ...
  async function gate(payload) {
    try {
      ...
      if (!r.json) return failClosed('control plane returned no decision');
  ...
  async function scanResponse(payload) {
    try {
      ...
      if (!r.json) return { leaked: true, decision: 'block', blocked: true, reasons: ['control plane returned no decision'], _failClosed: true };
      return r.json;
    } catch (e) {
