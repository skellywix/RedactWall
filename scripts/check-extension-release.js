...
function releaseReportFor({ now, extensionIdStatus, packaged, checks, problems }) {
  const packageManifest = packaged.packageManifest;
  return {
...
function checkExtensionRelease(opts = {}) {
  ...
  const extensionIdStatus = validateExtensionId(opts.extensionId || '');
  ...
  const releaseReport = releaseReportFor({ now, extensionIdStatus, packaged, checks, problems });
  ...
  return { ...packaged, reportPath, releaseReport };
}
