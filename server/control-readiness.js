...
function readinessArea({
  ...
  return {
    ...
    location: safeText(location, 'PromptWall control plane', 120),
    proofs: safeProofs,
...
function gatewayReadiness({ rows, policy, coverageReport }) {
  ...
  const playbook = [
    ...
    },
    {
      ...
      detail: 'Run a sanitized proxy smoke so PromptWall can see app-to-AI traffic before it leaves the network path.',
      command: 'npm run proxy:lab -- --sample --sentinel http://127.0.0.1:4000',
    ...
    },
    {
      ...
      detail: 'Move beyond monitor-only visibility by proving inline policy enforcement and approval release behavior.',
      command: 'node --test test/squid-icap-bridge.test.js',
...
function socReadiness({ policy, auditIntegrity, env, postureFeedSupported }) {
  ...
  const playbook = [
    ...
    },
    {
      ...
      validation: 'SIEM/SOAR receives promptwall.posture_snapshot without prompt text.',
      targetTab: 'audit',
    },
    {
